...
# This used to use `ping localhost`, but `ping` was removed from
# the KPI Docker image (shakes fist at David Burke).
# Other candidates include something like
# `wait-for-it --timeout=0 0.0.0.0:1337`, which is a non-standard utility (but
# one we explicitly install, for now!). `tail` should work until some eager
# person removes `coreutils` from the Docker image.
container_id=$(
    docker run --rm --detach "$latest_kpi_image" tail -f /dev/null
)
...
# `docker stop` would end up sending SIGKILL anyway, just after a useless
# 10-second wait, because `tail -f /dev/null` or whatever do-nothing process we launched
# inside the container is not designed to handle signals properly when running
# as init (PID 1)
